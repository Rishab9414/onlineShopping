...
@section('page-subtitle', 'Manage product brands')
...
window.renderMasterRow = (row) => `<tr class="hover:bg-slate-50">
    ...
    <td class="px-5 py-3.5 text-sm text-slate-600">${row.website ? `<a href="${row.website}" target="_blank" class="text-indigo-600 hover:underline">${row.website}</a>` : '—'}</td>
    <td class="px-5 py-3.5 text-sm text-slate-600">${row.country || '—'}</td>
...
@section('content')
<x-master-crud
    :base-url="route('admin.masters.brands.index')"
    module="brands"
    :columns="[['label'=>'Name'],['label'=>'Website'],['label'=>'Country'],['label'=>'Status']]"
>
    <x-slot:form>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="sm:col-span-2"><label class="block text-sm font-medium text-slate-700 mb-1">Brand Name *</label><input name="name" required class="admin-input text-sm"></div>
            <div><label class="block text-sm font-medium text-slate-700 mb-1">Website</label><input name="website" type="url" class="admin-input text-sm" placeholder="https://"></div>
            <div><label class="block text-sm font-medium text-slate-700 mb-1">Country</label><input name="country" class="admin-input text-sm"></div>
