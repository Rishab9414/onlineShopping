...
@section('title', 'Sizes')
@section('page-title', 'Size Master')
@section('page-subtitle', 'Manage product sizes')

@push('scripts')
<script type="module">
window.renderMasterRow = (row) => `<tr class="hover:bg-slate-50">
    <td class="px-5 py-3.5 font-medium text-slate-900">${row.name}</td>
    <td class="px-5 py-3.5 text-sm text-slate-600">${row.display_order ?? 0}</td>
...
@section('content')
<x-master-crud
    :base-url="route('admin.masters.sizes.index')"
    module="sizes"
    :columns="[['label'=>'Name'],['label'=>'Display Order'],['label'=>'Status']]"
>
    <x-slot:form>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div><label class="block text-sm font-medium text-slate-700 mb-1">Size Name *</label><input name="name" required class="admin-input text-sm"></div>
