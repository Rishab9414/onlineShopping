...
@section('title', 'Taxes')
@section('page-title', 'Tax Master')
@section('page-subtitle', 'Manage tax rates and HSN codes')

@push('scripts')
<script type="module">
window.renderMasterRow = (row) => `<tr class="hover:bg-slate-50">
    <td class="px-5 py-3.5 font-medium text-slate-900">${row.name}</td>
    <td class="px-5 py-3.5 text-sm text-slate-600">${row.percentage != null ? row.percentage + '%' : '—'}</td>
    <td class="px-5 py-3.5 text-sm text-slate-600">${row.hsn_code || '—'}</td>
...
@section('content')
<x-master-crud
    :base-url="route('admin.masters.taxes.index')"
    module="taxes"
    :columns="[['label'=>'Name'],['label'=>'Percentage'],['label'=>'HSN Code'],['label'=>'Status']]"
>
    <x-slot:form>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="sm:col-span-2"><label class="block text-sm font-medium text-slate-700 mb-1">Tax Name *</label><input name="name" required class="admin-input text-sm"></div>
            <div><label class="block text-sm font-medium text-slate-700 mb-1">Percentage *</label><input name="percentage" type="number" step="0.01" min="0" max="100" required class="admin-input text-sm"></div>
            <div><label class="block text-sm font-medium text-slate-700 mb-1">HSN Code</label><input name="hsn_code" class="admin-input text-sm"></div>
