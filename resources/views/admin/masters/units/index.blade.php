...
@section('title', 'Units')
@section('page-title', 'Unit Master')
@section('page-subtitle', 'Manage measurement units')

@push('scripts')
<script type="module">
window.renderMasterRow = (row) => `<tr class="hover:bg-slate-50">
    <td class="px-5 py-3.5 font-medium text-slate-900">${row.name}</td>
    <td class="px-5 py-3.5 text-sm text-slate-600">${row.short_name || '—'}</td>
...
@section('content')
<x-master-crud
    :base-url="route('admin.masters.units.index')"
    module="units"
    :columns="[['label'=>'Name'],['label'=>'Short Name'],['label'=>'Status']]"
>
    <x-slot:form>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div><label class="block text-sm font-medium text-slate-700 mb-1">Unit Name *</label><input name="name" required class="admin-input text-sm"></div>
            <div><label class="block text-sm font-medium text-slate-700 mb-1">Short Name</label><input name="short_name" class="admin-input text-sm" placeholder="e.g. kg, pcs"></div>
