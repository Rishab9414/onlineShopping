...
@push('scripts')
<script type="module">
    window.renderMasterRow = {!! $renderRowJs !!};
    @if(isset($readOnly) && $readOnly)
        new ReadOnlyTable({ baseUrl: '{{ $baseUrl }}' });
    @else
        new MasterCrud({ baseUrl: '{{ $baseUrl }}', module: '{{ $module ?? '' }}' });
    @endif
</script>
@endpush
